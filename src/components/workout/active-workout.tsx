                ...
                {Object.entries(exercisesByMuscleGroup).map(([muscleGroupName, exercises]) => {
                  ...
                  return <div key={muscleGroupName} className="space-y-0">
                    ...
                    <div className="border-2 border-t-0 rounded-b-lg border-border/80 overflow-hidden">
                      {incompleteExercises.map(exercise => <div key={exercise.id} className="ml-3 sm:ml-6 border-l-2 border-primary/20">
                        <UnifiedExerciseCard exerciseName={exercise.exercise_name} repsCount={exercise.reps_count || 1} repsUnit={exercise.reps_unit || "reps"} weightCount={exercise.weight_count || 0} weightUnit={exercise.weight_unit || "lbs"} leftWeight={exercise.left_weight} setCount={exercise.set_count} completedSets={exercise.completed_sets} note={exercise.note || undefined} muscleGroup={muscleGroupName} isCompleted={exercise.is_completed} type={exercise.type || 'exercise'} variant="added" onCompleteSet={!isCompleted ? decrement => handleCompleteSet(exercise.id, decrement) : undefined} onEdit={!isCompleted ? () => handleEditExercise(exercise.id) : undefined} onDelete={!isCompleted ? () => handleDeleteExercise(exercise.id) : undefined} disabled={isCompleted} workoutStarted={isStarted} />
                      </div>)}
                    </div>
                  </div>;
                })}
